// Flagz — flag guessing games. Reuses the shared rooms/account layer
// (account-ui.js handles login/profile/friends/menu); this module owns the
// landing/lobby/room flow, the prestart region+mode+difficulty picker, timing,
// and results. Gameplay lives in modes.js; country data in data.js.
//
// Multiplayer model (same as Atlaz): the host's `start` move (index 0) carries
...
const MAX_PLAYERS = 5;
const SESSION_KEY = 'flagz_session';
...
const app = {
  ...
  data: null,               // countries.json (loaded once at boot)
  phase: 'idle',            // idle | config | countdown | playing | results
  regionId: null, modeId: null, diffId: null,
  rounds: [],
  startAt: null,
  ctl: null,
  ...
  timerInt: null,
  rematching: false,
  cfgSel: { region: null, mode: null, diff: 'medium' },
  cfgStep: 'pick',
};
...
async function challengeFriend(friend) {
  try {
    ...
    triggerPush({ user_id: friend.id, title: 'Flagz challenge!', body: `${app.name} challenged you to Flagz.`, url: location.href.split('#')[0] }).catch(() => {});
    await enterRoom(room.code, 0, app.name, room);
  } catch (e) { $('lobby-error').textContent = e.message; }
}
...
function resetGame() {
  ...
  app.phase = 'idle';
  app.cfgStep = 'pick';
  app.regionId = null; app.modeId = null; app.diffId = null;
  app.rounds = [];
  app.startAt = null;
  app.results = {};
...
function handleMove(move) {
  ...
  if (move.type === 'start') {
    ...
    if (!app.data || !regionMetaOf(app.data, p.region) || !modeMeta(p.mode) || !diffMeta(p.diff)) return;
    beginGame(p.region, p.mode, p.diff, Number(p.startAt) || Date.parse(move.created_at) || Date.now());
    return;
  }
  if (move.type === 'result') {
...
// ---- Prestart: region + mode + difficulty picker ------------------------------------

const CFG_KEY = 'flagz.lastcfg';
function loadCfg() {
  try {
    const c = JSON.parse(localStorage.getItem(CFG_KEY) || '{}');
    if (app.data && regionMetaOf(app.data, c.region)) app.cfgSel.region = c.region;
...
function renderPrestart() {
  ...
  $('start-title').textContent = !host ? 'WAITING FOR THE HOST'
    : picking ? 'PICK A REGION, MODE & DIFFICULTY'
...
$('btn-start').addEventListener('click', async () => {
  if (!canConfigure() || app.phase !== 'config') return;
  const { region, mode, diff } = app.cfgSel;
  if (!cfgSummary()) return;
  if (app.cfgStep !== 'ready') { app.cfgStep = 'ready'; renderPrestart(); return; }
  $('btn-start').disabled = true;
  const startAt = Date.now() + 400;
  try {
    await app.conn.sendMove({ move_index: 0, player: app.seat, type: 'start', payload: { region, mode, diff, startAt } });
    updateRoomStatus(app.code, 'playing').catch(() => {});
    beginGame(region, mode, diff, startAt);
  } catch (e) {
...
function beginGame(regionId, modeId, diffId, startAt) {
...
function progressKey() { return `flagz.progress.${app.code}.${app.seat}`; }
...
async function persistResult() {
  ...
  const result = { scores, winner: winnerSeat(app.results, n), reason: 'done', region: app.regionId, mode: app.modeId, diff: app.diffId };
  try {
...
async function boot() {
  ...
  window.LB_CONFIG.historyDetail = (room) => {
    const r = room?.result || {};
    if (!app.data || (!r.region && !r.mode)) return '';
    return [regionMetaOf(app.data, r.region)?.label || r.region, modeMeta(r.mode)?.name || r.mode, diffMeta(r.diff)?.name]
      .filter(Boolean).join(' · ');
  };
