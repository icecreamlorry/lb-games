// Buffz — movie & TV trivia. Reuses the shared rooms/account layer
// (account-ui.js handles login/profile/friends/menu); this module owns the
// landing/lobby/room flow, the prestart FILTER+mode+difficulty picker, timing,
// and results. Gameplay lives in modes.js; title data in data.js.
//
// Multiplayer model (same as the siblings): the host's `start` move (index 0)
// carries { f: {type,decade,genre}, mode, diff, startAt }; everyone derives the
// same filtered pool and races identical seeded rounds; each seat submits ONE
// sparse `result` move (index 10+seat).
...
const MAX_PLAYERS = 5;
const SESSION_KEY = 'buffz_session';
...
const app = {
  ...
  data: null,               // titles.json (loaded once at boot)
  phase: 'idle',            // idle | config | countdown | playing | results
  f: null, modeId: null, diffId: null,
  rounds: [],
  startAt: null,
  ctl: null,
  ...
  timerInt: null,
  rematching: false,
  cfgSel: { f: { type: 'all', decade: 'all', genre: 'all' }, mode: null, diff: 'medium' },
  cfgStep: 'pick',
};
...
async function challengeFriend(friend) {
  try {
    ...
    triggerPush({ user_id: friend.id, title: 'Buffz challenge!', body: `${app.name} challenged you to Buffz.`, url: location.href.split('#')[0] }).catch(() => {});
    await enterRoom(room.code, 0, app.name, room);
  } catch (e) { $('lobby-error').textContent = e.message; }
}
...
function resetGame() {
  ...
  app.phase = 'idle';
  app.cfgStep = 'pick';
  app.f = null; app.modeId = null; app.diffId = null;
  app.rounds = [];
  app.startAt = null;
  app.results = {};
...
function validFilters(f) {
  return f && typeof f === 'object'
    ...
    && (f.decade === 'all' || decadeList(app.data).includes(f.decade))
    && (f.genre === 'all' || genreList(app.data).includes(f.genre));
}
...
function handleMove(move) {
  ...
  if (move.type === 'start') {
    ...
    if (!app.data || !validFilters(p.f) || !modeMeta(p.mode) || !diffMeta(p.diff)) return;
    beginGame(p.f, p.mode, p.diff, Number(p.startAt) || Date.parse(move.created_at) || Date.now());
    return;
  }
  if (move.type === 'result') {
...
// ---- Prestart: filters + mode + difficulty picker ------------------------------------

const CFG_KEY = 'buffz.lastcfg';
function loadCfg() {
  try {
    const c = JSON.parse(localStorage.getItem(CFG_KEY) || '{}');
    if (validFilters(c.f)) app.cfgSel.f = c.f;
...
function poolIds() { return filterIds(app.data, app.cfgSel.f); }

function buildCfgButtons() {
  // The three pool filters are dropdowns ("All" default) rather than a fixed
  // set list — the host mixes and narrows; the live count shows the result.
  const fill = (sel, opts) => {
    sel.innerHTML = '';
  ...
  fill($('f-decade'), [['all', 'All decades'], ...decadeList(app.data).map((d) => [d, d])]);
  fill($('f-genre'), [['all', 'All genres'], ...genreList(app.data).map((g) => [g, g])]);
  for (const key of ['type', 'decade', 'genre']) {
    $(`f-${key}`).addEventListener('change', (e) => {
      if (!canConfigure() || app.phase !== 'config') return;
      app.cfgSel.f = { ...app.cfgSel.f, [key]: e.target.value };
      saveCfg();
      renderPrestart();
    });
  }
...
function cfgSummary() {
  ...
  if (!m || !d) return '';
  return `${esc(filterLabel(app.cfgSel.f))} — ${esc(m.name)} — ${esc(d.name)}`;
}

// Spell out what the chosen difficulty does for the chosen mode — the dial is
// never silently inert (the family rule).
function diffEffect() {
  const m = modeMeta(app.cfgSel.mode);
  const d = diffMeta(app.cfgSel.diff);
  if (!m || !d) return '';
  ...
  if (isOrderMode(m.id)) {
    return `Sort ${Math.min(d.n, len)} at a time · ${rounds} round${rounds === 1 ? '' : 's'}`;
  }
...
function renderPrestart() {
  ...
  const picked = !!cfgSummary() && enough;
  ...
  $('start-title').textContent = !host ? 'WAITING FOR THE HOST'
    : picking ? 'FILTER THE POOL, PICK A MODE'
    : (n > 1 ? 'READY?' : 'WAITING FOR PLAYERS');

  // Sync the dropdowns (rematch/config restore) + live count.
  for (const key of ['type', 'decade', 'genre']) {
    const sel = $(`f-${key}`);
    sel.value = app.cfgSel.f[key];
    sel.disabled = !host;
  }
  ...
  const sampleNote = app.data?.sample ? '<br><span class="start-note">⚠ Sample data — run tools/build-data.mjs for the full TMDb pool.</span>' : '';
  if (!host) {
    ...
    $('start-info').innerHTML = (cfgSummary()
      ...
      : 'Filter the pool, then pick a mode and a difficulty.') + sampleNote;
  } else {
...
$('btn-start').addEventListener('click', async () => {
  if (!canConfigure() || app.phase !== 'config') return;
  const { f, mode, diff } = app.cfgSel;
  if (!cfgSummary() || poolIds().length < MIN_POOL) return;
  if (app.cfgStep !== 'ready') { app.cfgStep = 'ready'; renderPrestart(); return; }
  $('btn-start').disabled = true;
  const startAt = Date.now() + 400;
  try {
    await app.conn.sendMove({ move_index: 0, player: app.seat, type: 'start', payload: { f, mode, diff, startAt } });
    updateRoomStatus(app.code, 'playing').catch(() => {});
    beginGame(f, mode, diff, startAt);
  } catch (e) {
...
function beginGame(f, modeId, diffId, startAt) {
  app.f = f; app.modeId = modeId; app.diffId = diffId; app.startAt = startAt;
  setStatus('');
  const pool = filterIds(app.data, f);
  app.rounds = buildRounds(modeId, diffMeta(diffId), pool, Number(app.room?.seed) >>> 0, app.data.items);

  $('mode-chip').textContent = `${filterLabel(f)} · ${modeMeta(modeId).name}`;
...
function progressKey() { return `buffz.progress.${app.code}.${app.seat}`; }
...
async function onMyFinish(result) {
  ...
  app.submittedResult = true;
  app.ctl?.destroy(); app.ctl = null;
  renderReview(app.data, app.modeId, app.rounds, result);
...
async function persistResult() {
  ...
  const result = { scores, winner: winnerSeat(app.results, n), reason: 'done', f: app.f, mode: app.modeId, diff: app.diffId };
  try {
...
function spectate(seat) {
  ...
  renderReview(app.data, app.modeId, app.rounds, r);
...
async function boot() {
  ...
  window.LB_CONFIG.historyDetail = (room) => {
    ...
    return [filterLabel(r.f), modeMeta(r.mode)?.name || r.mode, diffMeta(r.diff)?.name]
      .filter(Boolean).join(' · ');
  };
  window.addEventListener('scroll', () => {
    if (window.scrollX || window.scrollY) window.scrollTo(0, 0);
  });
  try { app.data = await loadData(); } catch (e) { landingError(`Could not load title data (${e.message}).`); window.LBBoot?.done(); return; }
  buildCfgButtons();
  loadCfg();
