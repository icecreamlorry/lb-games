...
//
// Lexicorp is turn-based, so a notification when the opponent ends their turn
// is genuinely useful. This fires a local notification while the page is still
// alive in the background, and (when subscribed) the shared Edge Function can
// Web-Push the next player even with the browser closed. Notifications are
// shown through a service worker because Android Chrome does not support the
// `new Notification()` constructor.

import { VAPID_PUBLIC_KEY } from './config.js';
import { savePushSubscription, deletePushSubscription } from './net.js';

const MUTE_KEY = 'lexicorp_notify_muted';
const TAG = 'lexicorp-turn';
...
export async function showTurnNotification(body) {
  ...
  try {
    ...
    if (!reg) return;
    await reg.showNotification("Lexicorp — it's your turn", {
      body,
      tag: TAG,
      renotify: true,
      vibrate: [120, 60, 120],
