// "Your turn" notifications for when the tab is backgrounded. Identical in
// behaviour to the other LB Games titles — a local notification while the page
// is alive in the background, plus optional Web Push so a move reaches you with
// the browser closed. See wurdz/js/notify.js for the fuller commentary.

import { VAPID_PUBLIC_KEY } from './config.js';
import { savePushSubscription, deletePushSubscription } from './net.js';

const MUTE_KEY = 'weiqi_notify_muted';
const TAG = 'weiqi-turn';
...
export async function showTurnNotification(body) {
  ...
  try {
    ...
    if (!reg) return;
    await reg.showNotification("Weiqi — it's your turn", {
      body,
      tag: TAG,
      renotify: true,
      vibrate: [120, 60, 120],
